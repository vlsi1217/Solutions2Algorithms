...
public class OldSeamCarver {
	...
	private Color[][] colors; // RGB of pixels
	private double[][] energies; // Energy of pixels
	
	// constructor
	public OldSeamCarver(Picture picture) {
		this.pic = picture;
		...
		// (x, y) in 2D-arry => pixel (y, x) in picture
		for (int i = 0; i < row; i++)
			for (int j = 0; j < col; j++) 
				//get function takes width and height as parameters
				//i.e., col and row
	...
	private double xGradient(int x, int y) {
		int xR = colors[x][y-1].getRed() - colors[x][y+1].getRed();
		int xG = colors[x][y-1].getGreen() - colors[x][y+1].getGreen();
		int xB = colors[x][y-1].getBlue() - colors[x][y+1].getBlue();
		return (xR * xR + xG * xG + xB * xB);		
	}
	
	// calculate y-gradient of (x, y) in 2D array
	private double yGradient(int x, int y) {
		int yR = colors[x-1][y].getRed() - colors[x+1][y].getRed();
		int yG = colors[x-1][y].getGreen() - colors[x+1][y].getGreen();
		int yB = colors[x-1][y].getBlue() - colors[x+1][y].getBlue();
	...
	private void reBuildColorsEnergies() {
		this.col = pic.width();
		this.row = pic.height();
		
		this.colors = new Color[row][col];
	...
	public void removeHorizontalSeam(int[] a) {
		...
		this.pic = newpic;
		reBuildColorsEnergies(); //update colors and energies
	}
	...
	public void removeVerticalSeam(int[] a) {
		...
		for (int i = 0; i < row; i++) {
			for (int j = a[i]; j < col-1; j++)
				//remove pixel (a[i], i)
				//move pixels on its right one pixel left
